...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (path === "login" && req.method === "POST") {
        try {
            ...
            context.res = { status: 500, body: { error: "Error en servidor" } };
        }
        return;
    }
    ...
    if (path === "dashboard" && req.method === "GET") {
        try {
            ...
            context.res = { status: 500, body: { error: "Error analíticas" } };
        }
        return;
    }
    ...
    if (path === "venta" && req.method === "POST") {
        try {
            const item = { ...req.body, id: `ord_${Date.now()}`, tipo: "orden", fechaOrden: new Date().toISOString() };
